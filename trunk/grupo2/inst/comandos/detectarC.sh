			...
			if [ -f $file ];then
				...
				if [ "$validName" != "" ];then
					...
					if [ "$idmatch" == "" ]; then
						...
						loguearC.sh detectarC.sh A "No existe el usuario ${userid} en el archivo encuestadores, moviendo a rechazados"
					else
						...
						if [ $begindate -le $date -a $enddate -ge $date ]; then
							...
							loguearC.sh detectarC.sh I "Moviendo ${file} a preparados"
						else
							echo charly dice que mueve a rechazados por las fechas invalidas
							moverC.sh $file $GRUPO/rechazados/
							loguearC.sh detectarC.sh A "Fechas invalidas para ${userid}, moviendo a rechazados"
						fi
					fi
				else
					echo charly dice que mueve a rechazados por el nombre invalido
					moverC.sh $file $GRUPO/rechazados/
					loguearC.sh detectarC.sh A "Nombre invalido para ${file}"
				fi
			else
				echo charly dice que mueve $file a rechazados pues no es un archivo regular, pero pienso que es mejor ignorarlo
				moverC.sh $file $GRUPO/rechazados/
				loguearC.sh detectarC.sh A "${file} no es un archivo regular"
			fi
		done
